...
import { MoreVertical, Heart, SkipBack, Play, Pause, SkipForward, MoreHorizontal, Radio, Activity } from 'lucide-react';
...
export const PlayerView: React.FC<PlayerViewProps> = ({
  ...
  return (
    <div className="relative w-full h-full flex flex-col justify-between p-5 select-none text-white bg-[#0e0a17]/95 overflow-hidden">
      ...
      {/* Middle Section: Stacked Card Artwork */}
      <div className="relative z-10 my-auto py-2 flex items-center justify-center">
        <div className="relative w-full max-w-[290px] aspect-[1/1.1] group">
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#0d0918] via-transparent to-black/20" />
            ...
            {/* Tap to Live Hint on Hover */}
            <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10 text-[10px] text-purple-200 opacity-80 group-hover:opacity-100 flex items-center space-x-1">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-left">
              ...
              <div className="flex items-end space-x-0.5 h-4 mb-2">
                <span className={`w-0.5 bg-purple-400 rounded-full ${isPlaying ? 'h-3 animate-pulse' : 'h-1.5'}`} />
                <span className={`w-0.5 bg-purple-300 rounded-full ${isPlaying ? 'h-4 animate-pulse delay-75' : 'h-2'}`} />
                <span className={`w-0.5 bg-purple-400 rounded-full ${isPlaying ? 'h-2.5 animate-pulse delay-150' : 'h-1'}`} />
                <span className={`w-0.5 bg-purple-200 rounded-full ${isPlaying ? 'h-3.5 animate-pulse delay-100' : 'h-2.5'}`} />
                <span className={`w-0.5 bg-purple-400 rounded-full ${isPlaying ? 'h-2 animate-pulse delay-200' : 'h-1'}`} />
              </div>
      ...
      <div className="relative z-10 w-full space-y-4">
          ...
          >
            <Heart
              className={`w-5 h-5 transition-colors ${
                isLiked ? 'text-purple-400 fill-purple-400 filter drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]' : ''
              }`}
            />
          </button>
        ...
        <div
          onClick={onGoToLive}
          className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] backdrop-blur-xl transition-all cursor-pointer group"
        ...
        >
          ...
          <div className="px-2 py-0.5 rounded-full border border-white/20 text-[10px] font-mono font-medium text-white/70 group-hover:border-purple-400/50 group-hover:text-purple-200 transition-colors shrink-0">
            HQ
          </div>
        </div>
      </div>
